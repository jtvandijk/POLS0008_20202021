...
| 10   | Inspect your data in a texteditor such as [Wordpad](https://en.wikipedia.org/wiki/WordPad) or [Textedit](https://support.apple.com/en-gb/guide/textedit/welcome/mac) to make sure the file you created is indeed a comma-separated file.
...
#### Questions {-}
1. Inspect the columnames. Why do you think that they slightly differ from the ones that we saw in Microsoft Excel?
...
#### Questions {-}
1. How can we get the same data frame with selected columns (`midyear_sel`) using the `base` R syntax we have been using until now?
2. How can we get the same data frame with filtered rows (`midyear_fil`) using the `base` R syntax we have been using until now?
...
This looks much better. We can now try to calculate the median and the mean again:
...
#### Additional tasks {-}
If you found these assignments and questions relatively easy and want to be challenged a little more:

1. Download the mid-year population estimates for 2018 to your computer (`Mid-2018: SAPE21DT3a`).
2. Prepare the file and import into R.
3. Take the necessary steps in R to make sure that all numeric variables are also recognised by R as numeric variables.
4. Join the 2018 and 2019 midyear population data set by using the `left join()` function from the `tidyverse()` package, e.g. something like: `midyear <- left_join(midyear2018,midyear2019,by='msoa_code')`.
5. Compare the population change between 2018 and 2019 by creating a scatterplot.
