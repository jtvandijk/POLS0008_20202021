...
5. Once opened your file should look similar as to the screenshot below.
...
1. Open the downloaded file in Microsoft Excel and navigate to the `Mid-2019 Persons` tab.
...
3. Scroll down all the way to the bottom of the file and whilst holding the **shift** button, click on the last value in the *All Ages* column. This value should be 9,711. All data should now be selected.
...
The column names may seem a little complicated but in fact simply refer to some of the administrative geographies that the ONS uses for their statistics. Don't worry if you do not fully understand these as they are notoriously complicated and confusing!
...
Great. This is all looking very good but we are not completely there yet. Now we sourced our data and read it into R, we need to do some final preparations so that our data is analysis ready.
...
<div class="note">
**Note**<br/>
The `tidyverse` package is in fact a collection of packages that are designed for data science tasks. Where in many cases different packages work all slightly differently, all packages of the `tidyverse` share the underlying design philosophy, grammar, and data structures. The `ggplot2` package that you worked with last week is in very fact one of the core package of the `tidyverse`. Because the `tidyverse` consists of multiple packages, it may take a little while before everything is installed so be patient! For more information on `tidyverse`, have a look at https://www.tidyverse.org/.
</div>
