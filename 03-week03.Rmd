...
For this tutorial we start by using the `London.Pop` object again that we created during our tutorial in [Week 01](understanding-data.html#r-script) and that we also briefly used last week. You may still have it loaded into your R workspace if your RStudio Server session is still active. To check if you do, you can use the `ls()` command to get a list of all objects that are available in your RStudio session. Type this into the command line and see if `London.Pop` is printed. 
...
Now are data are loaded again, we can start by recreating the simple [scatter graph](https://en.wikipedia.org/wiki/Scatter_plot) we created last week using the `plot()` function.
...
3. How can we add a title? And how do we change the colour of the points? 

Another common method to visualise your data is by using a [line chart](https://en.wikipedia.org/wiki/Line_chart). Line charts are particularly useful when you want to compare change over time, for example. A line chart can also be relatively easily created in R. Let's try it by charting the population change over time for one of London's boroughs: [Hackney](https://en.wikipedia.org/wiki/London_Borough_of_Hackney). 
...
We should now be left with a dataframe that only contains one row of data: the data for the borough of Hackney. This does raise a question now: how do we include all the variables into our plot as all our data are spread out over different columns (i.e. *Persons1801*,*Persons1811*, etc.)? Ideally we would **flip** the data set around so that all the data (i.e. *Persons1801*,*Persons1811*, etc.) would be in one column. Well, luckily we can actually do that by [transposing](https://en.wikipedia.org/wiki/Transpose) our data set. Transposing is a mathemathical operation to flip a matrix over its diagonal. Effectively, it turns rows into columns and columns into rows. In R we can do this using the `t()` function.
...
# plot the population change over time in the London borough of Hackney 
plot(London.Hackney.t, type='b', col='blue')
```

#### Questions {-}
1. Why did we use quotation marks when we selected Hackney (i.e. `London.Pop[London.Pop$Area.Name=='Hackney',3:24]`)?
2. Why did we slice our data frame when we selected the data for Hackney (i.e. `London.Pop[London.Pop$Area.Name=='Hackney',3:24]`)?
3. How can we change the colour of the information in our graph?
...
Last week you also got introduced to the box and whisker plot. A box and whisker plot is a method for visualising your data by their quartiles with the whiskers (the lines extending from the box) incidating the variability of the outside the lower and upper quartile. Typically outliers are plotted as individual points. Let's go back to our full data set and make a boxplot for one of the population variables in the London population data set.
...
The standard `plot()` and `boxplot` functions offers a huge number of options for customisation. A useful option is to combine multiple graphs into one figure to allow for easy comparison. We can do this by setting some of the graphical parameters and requesting that our graphs are plotted together.
...
<div class="note">
**Note**<br/>
The specification given to the `par()` function only works once: it is not an option that you switch on and from now on all plots will be plotted together. This means that if you run this code line by line, you will not get the result you want. In order to get both boxplots into one figure, highlight all three lines and then run the code.
</div>
...
The last type of plot we will introduce to you today is called a [histogram](https://en.wikipedia.org/wiki/Histogram). As explained in the lecutre video, a histogram is a graphical display of data using bars of different heights. It is therefore quite similar to a bar chart, however, with a histogram we **group** our values into bins. We can create in histogram in R using the `hist()` function.
...
Currently R automatically picks the widht of the bins in which the data are grouped. To change the width of these bins, we can specify the `breaks` parameter:
...
As there are many hundres of R packages, these hese are not installed automatically. This means that every additional package we want to use needs to be downloaded and then we need to load it into R in order to use it. To download and install the `ggplot2` package type the following:
...
You can already see that this plot is looking a bit nicer than the one we created with the base `plot()` function used above. Within the `geom_point()` brackets you can alter the appearance of the points in the plot. 
...
2. What other parameters can you pass to the `geom_point()` function? Tip: use `?ggplot2::geom_point` to find out.

Rather than colouring your points by one colour, you can also colour the points according to another variable. You can do this by adding the desired variable into the `aes()` section after `geom_point()`. Here we will do this to indicate the size of the population in 2011 as well as the relationship between the size of the population in 19811 and 1911.
...
# add geom to our 'p' object, adjust the bindwidth settings
...
The crucial part of the code snippet above is what's included in the square brackets `[ ]`. We are subsetting the `London.Ambulance` object, but instead of telling R what column names or numbers we require, we are requesting all rows in the *Bor_Code* column that contain `00AG`. `00AG` is a text string so it needs to go in speech marks `''` (or `""`) and we need to use two equals signs `==` in R to mean "equals to". A single equals sign `=` is another way of assigning objects. Although `=` works the same way as `<-`, it is much less widely used for this purpose because it is also used when parameterising functions. Let's quickly compare our original `London.Pop` object with our newly created `camden` object:
